...
# 4. Now to break the data up into the three zones. The middle zone contains logs, so I'll need to make the log location's NA's:
...
# Middle Zone:
```{r}
vegdata_M <- vegdatamerged[(vegdatamerged$Zone == "M"),]
```
# Lower Zone:
```{r}
vegdata_L <- vegdatamerged[(vegdatamerged$Zone == "L"),]
```
...
# Upper Zone Normality:
```{r}
#QQ-Plot:
qqnorm(vegdata_U$Total_Veg, pch = 1, frame = FALSE)
qqline(vegdata_U$Total_Veg, col = "steelblue", lwd = 2)

#histogram
hist(vegdata_U$Total_Veg)
```
# Middle Zone Normality:
```{r}
#QQ-Plot:
qqnorm(vegdata_M$Total_Veg, pch = 1, frame = FALSE)
qqline(vegdata_M$Total_Veg, col = "steelblue", lwd = 2)

#histogram
hist(vegdata_M$Total_Veg)
```
# Lower Zone Normality:
```{r}
#QQ-Plot:
qqnorm(vegdata_L$Total_Veg, pch = 1, frame = FALSE)
qqline(vegdata_L$Total_Veg, col = "steelblue", lwd = 2)

#histogram
hist(vegdata_L$Total_Veg)
```
