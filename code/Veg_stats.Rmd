...
vegdata_U <- vegdatamerged[(vegdatamerged$Zone == "U" & vegdatamerged$Log_Presence != "reference"),] %>% 
  ...
  ungroup() %>% 
  filter(!(Date == "2022-June")) %>%  #this just removes the June count_difference data bc it is a repeat for the august data and we only need one calculation
...
```{r}
#QQ-Plot:
qqnorm(vegdata_U$mean_cover, pch = 1, frame = FALSE)
qqline(vegdata_U$mean_cover, col = "steelblue", lwd = 2)
