...
VegDataMerged_mod_U <- VegDataMerged[(VegDataMerged$Date == "2022-June" & VegDataMerged$Zone == "U" &
                                  !(VegDataMerged$Transect_ID == "C6-L174" & 
                                      VegDataMerged$Quadrat == 3)), ] %>% 
  select(-c('GrGR','LeTr','MixedSeedlings','SaPa','SaDe','PicklePups'))

### Not sure if I can run this or not!!!
adonis2(VegDataMerged_mod_U$Total_Veg ~ Shoreline_End*Log_Presence, 
        data=VegDataMerged_mod_U,
        method = "bray")
...
```{r}
#First, need to remove these previous columns that were combined back in the beginning and remove the weirdo in quad 3 for C6-L174:
VegDataMerged_mod_M <- VegDataMerged[(VegDataMerged$Date == "2022-June" & VegDataMerged$Zone == "M"), ] %>% 
  select(-c('GrGR','LeTr','MixedSeedlings','SaPa','SaDe','PicklePups'))

### Not sure if I can run this or not!!!
adonis2(VegDataMerged_mod_M$Total_Veg ~ Shoreline_End*Log_Presence, 
        data=VegDataMerged_mod_M,
        method = "bray",
...
```{r}
#First, need to remove these previous columns that were combined back in the beginning and remove the weirdo in quad 3 for C6-L174:
VegDataMerged_mod_L <- VegDataMerged[(VegDataMerged$Date == "2022-June" & VegDataMerged$Zone == "L"), ] %>% 
  select(-c('GrGR','LeTr','MixedSeedlings','SaPa','SaDe','PicklePups')) %>% 
  mutate(Shoreline_End = factor(Shoreline_End, levels = c("west","east")),
         Log_Presence = factor(Log_Presence, levels = c("log", "no log")))

### Not sure if I can run this or not!!!
adonis2(VegDataMerged_mod_L$Total_Veg ~ Shoreline_End*Log_Presence, 
        data=VegDataMerged_mod_L,
        method = "bray",
        na.action=na.exclude)

sum(is.na(VegDataMerged_mod_L))
```