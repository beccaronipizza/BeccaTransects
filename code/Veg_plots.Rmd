...
```{r}
## the code below is form the veg_stats file.  Might eb able to use snippets fo this to plot!!
...
TotalVeg_change_barplot <- ggplot(TotalVeg_change, 
  ...
  scale_x_discrete(labels = c(U = "Upper", M = "Middle", L = "Lower")) +
  coord_cartesian(ylim = c(-10, 100)) + #sets the limit of the y-axis to show a more dramatic difference bw variables
  labs( 
    title = "Mean % Change in Vegetation Cover",
  ...
  theme(axis.title = element_text(face = "bold", size=14),
        ...
        strip.text = element_text(size = 13, color = "black")) +
  geom_text(aes(label = mean_cover), #geom text is for the labels over the bars
                vjust = -2.5,  #higher values shift text down
                hjust = 0.5, #higher values shift text horizontally left
...
SaPa_change_barplot <- ggplot(SaPa_change, 
  ...
  facet_grid(Log_Presence ~ Shoreline_End) +
  scale_x_discrete(labels = c(U = "Upper", M = "Middle", L = "Lower")) +
  coord_cartesian(ylim = c(-10, 30)) + #sets the limit of the y-axis to show a more dramatic difference bw variables
  labs( 
