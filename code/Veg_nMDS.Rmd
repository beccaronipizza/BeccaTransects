...
nmds_M <- metaMDS(VegDataSpecies.hel_M, autotransform = F, k = 2, trymax = 100) #autotransform = F so that it doesn't revert back to the default transformation *bc we are using the hellinger method.
nmds_M

save(nmds_M, file ="../output_data/nmds_M.Rds")

load("output_data/nmds_M.RData")
```
...
nmds_L <- metaMDS(VegDataSpecies.hel_L, autotransform = F, k = 2, trymax = 100) #autotransform = F so that it doesn't revert back to the default transformation *bc we are using the hellinger method.
nmds_L

save(nmds_L, file ="../output_data/nmds_L.Rds")

load("output_data/nmds_L.RData")
```
...
p3 <- ggplot() +
  geom_point(data = mds.coords1_U, 
             aes(x=NMDS1, y=NMDS2, 
                color = Log_Presence,
                shape = Shoreline_End),
             alpha = 0.5) +          #keep this detail outside of the aes() if you don't want it to show up in the legend
  scale_color_manual(values = cbp) +   #override base ggplot colors with colorblind friendly colors
...
EWLNL_M_plot <- ggplot() +
  ...
  #             size = 0.6) +
               labs(title = "Middle Zone nMDS",
                    subtitle = "stress = 0.07531825, bray") +
  geom_abline(intercept=0, slope=0, linetype = "dashed", size = 0.8, color = "gray")+
  geom_vline(aes(xintercept=0), linetype = "dashed", size = 0.8, color = "gray")+
  labs(title = "Middle Zone nMDS") +
  theme_bw() +
...
EWLNL_L_plot <- ggplot() +
  ...
  subtitle = "stress = 0.02575065 , bray") +
  geom_abline(intercept=0, slope=0, linetype = "dashed", size = 0.8, color = "gray")+
  geom_vline(aes(xintercept=0), linetype = "dashed", size = 0.8, color = "gray")+
