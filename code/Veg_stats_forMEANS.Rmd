...
TotalVeg_change <- vegdatamerged[(vegdatamerged$Log_Presence != "reference"),] %>% #remove reference logs from calculation
  ...
  #filter(Transect_ID != "C1-BW12") %>%  #removing this transect to see if it makes a difference in the model output
  #filter(Transect_ID != "C1-L11") %>%   #removing this transect to see if it makes a difference in the model output
...
emmeans(Interactive_glmm3, pairwise ~ Shoreline_End|Zone) #run this if there is an interactive effect, or if you get a significant main effect for a variable with more than two df's
...
# Total SpFo Change:   IGNORE!! UPDATED DOWN BELOW!!
```{r}
...
#SaPa GLMM with Julie (copied from GLMM with Julie above. Just applying her methods. She didn't actually help me with this specific analysis.)
```{r}
...
emmeans(Interactive_glmm_SaPa, pairwise ~ Zone) #run this if there is an inteactive effect, or you have a main effect of a variable with mroe than two factors
check_normality(Interactive_glmm3) #library(performance) not for glmer.nb models. Use for lmer.

sim_res <- simulateResiduals(Interactive_glmm_SaPa) # generates fitted model residuals
plot( sim_res ) # draws diagnostic plots along with diagnostic tests
testDispersion ( sim_res )
```
...
#SpFo GLMM with Julie (copied from GLMM with Julie above. Just applying her moethods. She didn't actually help me with this specific analysis.):
```{r}
...
emmeans(Interactive_glmm_SpFo, pairwise ~ Zone) #run this if there is an inteactive effect
check_normality(Interactive_glmm) #library(performance) not for glmer.nb models. Use for lmer.
