...
GIS_PercChange_byZandS <- GISvegdata_percent %>% 
  filter(Zone !="All") %>% 
  filter(Initial_year == "2021" & Final_year == "2023") %>% 
  mutate(Shore_end = factor(Shore_end, levels = c("west","east")),
         Zone = factor(Zone, levels = c("Upper","Middle","Lower"))) %>% #make it so that facet grid orders shoreline end from west to east
  select(Zone, Shore_end, Percent_change, Initial_year) %>% 
  mutate(across(where(is.numeric), ~round(., 0))) 
...
GIS_veg_PercentChange <- ggplot(GIS_PercChange_byZandS,
  ...
  scale_x_discrete(labels = c(U = "Upper", M = "Middle", L = "Lower")) +
  coord_cartesian(ylim = c(-4, 53)) + #sets the limit of the y-axis to show a more dramatic difference bw variables
  labs( 
...
GIS_PercChange_byZandS <- GISvegdata_percent %>% 
  filter(Zone !="All") %>% 
  filter(Initial_year == "2021" & Final_year == "2022") %>% 
  mutate(Shore_end = factor(Shore_end, levels = c("west","east")),
         Zone = factor(Zone, levels = c("Upper","Middle","Lower"))) %>% #make it so that facet grid orders shoreline end from west to east
...
GIS_veg_PercentChange_yr1<- ggplot(GIS_PercChange_byZandS,
  ...
  facet_grid(~ Shore_end) +
  scale_x_discrete(labels = c(U = "Upper", M = "Middle", L = "Lower")) +
  coord_cartesian(ylim = c(-12, 80)) + #sets the limit of the y-axis to show a more dramatic difference bw variables
  labs( 
  ...
  theme(axis.title = element_text(face = "bold", size=12),
        ...
        plot.tag.position = "right",
        strip.text = element_text(size = 13, color = "black")) +
  geom_label(aes(label = Percent_change, y = -17),  # Adjust the x-coordinate
           vjust = -0.5,
           color = "black",
           position = position_dodge(0.85),
           size = 3.25,
...
GIS_veg_PercentChange_yr2<- ggplot(GIS_PercChange_byZandS_yr2,
  ...
  facet_grid(~ Shore_end) +
  scale_x_discrete(labels = c(U = "Upper", M = "Middle", L = "Lower")) +
  coord_cartesian(ylim = c(-23, 27)) + #sets the limit of the y-axis to show a more dramatic difference bw variables
  labs( 
    title = "Sears Point % Veg Cover Change After 1 Year via GIS & Aerial Imagery",
  ...
  theme(axis.title = element_text(face = "bold", size=12),
        ...
        plot.tag.position = "right",
        strip.text = element_text(size = 13, color = "black")) +
  geom_label(aes(label = Percent_change, y = -25),  # Adjust the x-coordinate
           vjust = -0.5,
           color = "black",
           position = position_dodge(0.85),
           size = 3.25,
...
GIS_PercChange_byZandS_v3 <- GISvegdata_percent %>% 
  ...
  mutate(Shore_end = factor(Shore_end, levels = c("west","east")),
         ...
         Initial_year = factor(Initial_year, levels = c("2021", "2022"))) %>% #make it so that facet grid orders shoreline end from west to east
  select(Zone, Shore_end, Percent_change, Initial_year) %>% 
  mutate(across(where(is.numeric), ~round(., 0))) %>% 
  group_by(Shore_end, Zone)
...
GIS_veg_PercentChange_v3<- ggplot(GIS_PercChange_byZandS_v3,
  ...
  facet_grid(~ Shore_end) +
  scale_x_discrete(labels = c(U = "Upper", M = "Middle", L = "Lower")) +
  coord_cartesian(ylim = c(-25, 55)) + #sets the limit of the y-axis to show a more dramatic difference bw variables
  labs( 
    title = "Sears Point % Veg Cover Change via GIS & Aerial Imagery",
    subtitle = "Year 1 and Year 2 Comparison",
    x = "Marsh Zone",
    y ="Total Vegetation Cover (%)",
   fill = "Year",
   #caption = "*Error bars reflect standard error",
  ...
  theme(axis.title = element_text(face = "bold", size=12),
        ...
        plot.tag.position = "right",
        strip.text = element_text(size = 13, color = "black")) +
  geom_label(aes(label = Percent_change, y = -29),  # Adjust the x-coordinate
           vjust = -0.5,
           color = "black",
           position = position_dodge(0.85),
           size = 3.25,
