...
```{r}
GIS_veg_PercentChange_byS <- ggplot(GIS_PercChange_byZandS,
                               mapping = aes(x = year,
                                             y = Percent_change,
                                             group = Shore_end,
                                             fill = Shore_end)) +
  ...
  facet_grid(~ Zone) +
  #scale_x_discrete(labels = c(U = "Upper", M = "Middle", L = "Lower")) +
  coord_cartesian(ylim = c(30, 60)) + #sets the limit of the y-axis to show a more dramatic difference bw variables
  labs( 
    title = "Sears Point Percent Veg Cover Change (%) via GIS & Aerial Imagery",
    subtitle = "From December 2021 to August 2023",
    x = "Shoreline End",
    y ="Total Vegetation Cover (%)",
   fill = "Shoreline End",
   #caption = "*Error bars reflect standard error",
    tag = "Veg_plots_GIS.Rmd-GIS_veg_PercentChange_byS") +
  theme_bw(base_size = 10) +
  theme(axis.title = element_text(face = "bold", size=12),
